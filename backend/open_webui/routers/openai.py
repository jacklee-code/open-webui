...
async def get_headers_and_cookies(
    ...
    headers = {
        ...
        **(
            {
                'HTTP-Referer': 'https://github.com/jacklee-code/open-webui',
                'X-Title': 'JackAI',
            }
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=detail if detail else 'JackAI: Server Connection Error',
            )
    ...
    if url_idx is None:
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail='JackAI: Server Connection Error')
            except Exception as e:
...
async def proxy(path: str, request: Request, user=Depends(get_verified_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=r.status if r else 500,
            detail='JackAI: Server Connection Error',
        )
    finally:
        if not streaming:
            await cleanup_response(r)
